...
<html lang="en">
    ...
    <style>
      ...
      .hero {
        ...
        background-image:  linear-gradient(
          ...
          rgba(68, 160, 141, 0.3),
          rgba(9, 54, 55, 0.5)
          ), url(https://images.unsplash.com/photo-1652447385309-0e2d0b9edea9?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=871&q=80);
  ...
  <?php if(isset($_SESSION['completeReservation'])){ ?>
    ...
    z-index: 1;transition: All 300ms;' class='Successful-reserved position-absolute mx-auto mt-5 w-50 p-4'>
      ...
      <?php } ?>
  <!-- //////////////////////////////////// -->
  <section class="w-100 hero d-flex justify-content-center align-items-center">
  <form class='first-form w-50 p-5 text-light d-flex justify-content-center flex-column' action='http://localhost/trains/user/index' method='POST'>
          <div class="row mb-5">
            <div class="form-outline mb-4"">
              <div class="form-outline">
								<label class="form-label">departure  Trip</label>
		            <input type="text" name="departureStationTripSearch" class="form-control">
              </div>
            </div>
            <div class="form-outline mb-4"">
              <div class="form-outline">
						   <label class="form-label">arrival  Trip</label>
...
if(isset($_POST['submit']))
{
    ...
    {
      if($_POST['departureStationTripSearch'] == $Trip['departureStationTrip'] && $arrivalSearch == $Trip['arrivalStationTrip'] && $Trip['AvailableSeatsTrip'] > 0 && $Trip['status'] == 'active')
      {
          $dateTrip = str_replace("T"," ",$Trip['dateTrip']);
          if($dateTrip >= $dateNow)
          {
            echo "
            <form action='http://localhost/trains/user/booking' method='POST'>
      ...
      if($Found == false)
      {
        echo "<p class='mt-4 fs-2 text fw-bolder'>no result found</p> ";
      }
    }
?>
