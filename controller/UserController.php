		...
		if(isset($_POST['submit']))
{
		...
		if(!empty($_POST['departureStationTripSearch']) && !empty($_POST['arrivalStationTripSearch']))
  {
    ...
    {
      if($departureSearch == $Trip['departureStationTrip'] && $arrivalSearch == $Trip['arrivalStationTrip'] && $Trip['AvailableSeatsTrip'] > 0 && $Trip['status'] == 'active')
      {
    ...
    session_start();
		...
		if(isset($_POST['complete']) && !empty($_POST['name']) && !empty($_POST['email']) && !empty($_SESSION["depart"]))
		{
			...
			header('Location: http://localhost/trains/user');
		}
	...
	public function logout()
	{
		...
		header('Location: http://localhost/trains/user/index');
	}
		...
		if(isset($_POST['submit']))
		{
			if(!empty($_POST['name']) && !empty($_POST['email']) && !empty($_POST['password']) && !empty($_POST['passwordRepeat']))
			{
				if($_POST['password'] == $_POST['passwordRepeat'])
				{
					...
					header("Location: http://localhost/trains/user/userlogin");
				} else {
					$pass = true;
				}
			} else 
	...
	public function save()
	{
		...
		$Trip->save();
		header("Location: http://localhost/trains/Trip/index");
	}
